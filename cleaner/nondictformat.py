...
problems = []
problemtypes = {'verb': 0 , 'plural': 0, 'space': 0}
...
for k, e in sorted(ENTRIES.items()):
    ...
    if regex.match(rp_space,e.getOrig()):
        problems.append([e.getOrig() , 'space' , bool(regex.match(rp_space,e.getOrig()))])
        problemtypes['space'] = problemtypes['space'] + 1
